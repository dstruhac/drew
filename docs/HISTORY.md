# Klopi — podrobná historie a rozhodnutí (archiv)

**Tohle je archiv, ne živý přehled.** Na rozdíl od `docs/PROJECT.md`
se tenhle soubor **nenačítá automaticky** na začátku každé session (v
`CLAUDE.md` není `@docs/HISTORY.md`) — je tu proto, aby žádná podrobnost
nezmizela, ale nikdo za ni neplatil tokeny, když ji zrovna nepotřebuje.

**Kdy sem sáhnout:** když narazíš na chybu/chování, co působí povědomě
(typicky "permission denied" na novou tabulku — tenhle vzorec se
opakoval už 9×, viz sekce Grants níže), když potřebuješ přesně
zrekonstruovat, PROČ appka dělá něco konkrétního způsobem, jakým to
dělá, nebo když tě zajímá historie/proces rozhodnutí u konkrétní
featury. Pro běžnou práci (co appka umí teď, jak navázat, aktuální
datový model) čti `docs/PROJECT.md` — ten shrnuje jen současný stav a
na relevantní místa v tomhle souboru odkazuje.

Obsah odpovídá dřívějšímu jedinému `docs/PROJECT.md` (rozděleno
16.9.2026, appka do té doby rostla jako jeden nepřetržitě aktualizovaný
dokument — viz `PROJECT.md` sekce "Údržba dokumentace" pro důvod a
konvenci rozdělení). Datace/chronologie uvnitř je beze změny.
